...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `-1 @${num.split('@')[0]} já foi tarde🗿`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⏳Tô fazendo caraí⏳',
				success: 'Pronto🤙',
				error: {
					stick: 'é "sticker"* burro ',
					Iv: '❄️ Link inválido ❄️'
				},
				only: {
					group: '❄️ Isso por acaso é um grupo? ❄️',
					ownerG: '❄️ Ala membro comum tentando usar esse comando kkj ❄️',
					ownerB: '❄️ Comando usado só pelo Guuh ❄️',
					admin: '❄️ Ala membro comum tentando usar comando dos adm kkj ❄️',
					Badmin: '❄️ Da adm pro bot ai primeiro 🗿🤙 ❄️'
				}
			}
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❄️ Deu ruim, tenta dnv ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❄️ responder stickernya hum ❄️')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❄️ n deu pra converter adesivos em imagens ❄️')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativou esse modo simih dnv? pqp em (ativo) ❄️')
